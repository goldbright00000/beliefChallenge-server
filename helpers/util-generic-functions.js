...
const dbFindOne = async (model, args) => (
    await model.findOne(args.query, args.parameterToGet).sort(args.sort)
        .then((data) => ({
            status: true,
            data
        }))
...
const dbFindWithCount = async (model, args) => {
    // console.log(args)
    return await model.find(args.query, args.parameterToGet).limit(args.extra.limit).skip(args.extra.skip).sort(args.sort)
        .then((data) => ({
            status: true,
            data
        }))
...
const dbAggregate = async (model, args) => (
    // console.log(model, args.query, args.count)
    await model.aggregate(args.query, args.parameterToGet)
        .then((data) => ({
            status: true,
            data
        }))
